...
<div>
	...
	<t-modal ref="modal" class="curdmodel">
        <form class="bg-white rounded px-8 pt-6 pb-8 mb-4" @submit.prevent="storeOrUpdate" @keydown="form.onKeydown($event)">
            <div class="p-3">
                <h2 class="mb-2">{{ updateData ? 'Update' : 'Create' }} Issue Type</h2>
...
export default {
    ...
    methods:{
    	...
    	async storeOrUpdate () {
             // Submit the form.
            if(this.updateData) {
                const response = await this.form.put('/api/issue_types/'+this.form.id);
                if (response.status === 200) {
                    this.cleanForm();
                    showMessage(response.status, 'Issue Type updated successfully');
                }
            }else {
                const response = await this.form.post('/api/issue_types');
                if (response.status === 200) {
                    this.cleanForm();
                    showMessage(response.status, 'Issue Type created successfully');
                }
            }
        },
        ...
        async getData(url = this.url, options = this.tableProps) {
            ...
            if(response.status === 200) {
                this.issue_types = response.data;
            }
        }
    },
    created(){
        var self = this;
        axios.get('/api/issue_types/create').then(function (response) {
