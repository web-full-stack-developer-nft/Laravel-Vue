...
<div>
	...
	<t-modal ref="modal">
        <form class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4" @submit.prevent="storeOrUpdate" @keydown="form.onKeydown($event)">
            <div class="p-3">
                <h2 class="mb-2">{{ updateData ? 'Update' : 'Create' }} Department</h2>
...
export default {
    ...
    methods:{
    	...
    	async storeOrUpdate () {
             // Submit the form.
            if(this.updateData) {
                const response = await this.form.put('/api/departments/'+this.form.id);
                if (response.status === 200) {
                    this.cleanForm();
                    showMessage(response.status, 'Department updated successfully');
                }
            }else {
                const response = await this.form.post('/api/departments');
                if (response.status === 200) {
                    this.cleanForm();
                    showMessage(response.status, 'Department created successfully');
                }
            }
        },
    ...
    created(){
        var self = this;
        axios.get('/api/departments/create').then(function (response) {
