...
<div>
	...
	<t-modal ref="modal" class="curdmodel">
        <form class="bg-white rounded px-8 pt-6 pb-8 mb-4" @submit.prevent="storeOrUpdate" @keydown="form.onKeydown($event)">
            <div class="p-3">
                <h2 class="mb-2">{{ updateData ? 'Update' : 'Create' }} Status</h2>
...
export default {
    ...
    methods:{
    	...
    	async storeOrUpdate () {
             // Submit the form.
            if(this.updateData) {
                const response = await this.form.put('/api/statuses/'+this.form.id);
                if (response.status === 200) {
                    this.cleanForm();
                    showMessage(response.status, 'Status updated successfully');
                }
            }else {
                const response = await this.form.post('/api/statuses');
                if (response.status === 200) {
                    this.cleanForm();
                    showMessage(response.status, 'Status created successfully');
                }
            }
        },
    ...
    created(){
        var self = this;
        axios.get('/api/statuses/create').then(function (response) {
