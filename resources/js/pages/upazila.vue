...
<div>
	...
	<t-modal ref="modal" class="curdmodel">
        <form class="bg-white rounded px-8 pt-6 pb-8 mb-4" @submit.prevent="storeOrUpdate" @keydown="form.onKeydown($event)">
            <div class="p-3">
                <h2 class="mb-2">{{ updateData ? 'Update' : 'Create' }} Upazila</h2>
                <div class="my-1" v-for="(value,name, index) in form.originalData" :key="index">
                    <div class="mb-4" v-if="name === 'district'">
...
export default {
    ...
    methods:{
    	...
    	async storeOrUpdate () {
             // Submit the form.
            if(this.updateData) {
                const response = await this.form.put('/api/upazilas/'+this.form.id);
                if (response.status === 200) {
                    this.cleanForm();
                    showMessage(response.status, 'Upazila updated successfully');
                }
            }else {
                const response = await this.form.post('/api/upazilas');
                if (response.status === 200) {
                    this.cleanForm();
                    showMessage(response.status, 'Upazila created successfully');
                }
            }
        },
        ...
        async fetchDistricts() {
            const response = await axios.get('/api/districts/all');
            if(response.status === 200) {
                this.districts = await response.data.map((district) => {
                    return {
                        value: district.id,
                        text: district.name
                    }
    ...
    created(){
        this.fetchDistricts();
        var self = this;
        axios.get('/api/upazilas/create').then(function (response) {
