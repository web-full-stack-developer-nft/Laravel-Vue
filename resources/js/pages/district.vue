...
<div>
	...
	<t-modal ref="modal" class="curdmodel">
        <form class="bg-white rounded px-8 pt-6 pb-8 mb-4" @submit.prevent="storeOrUpdate" @keydown="form.onKeydown($event)">
            <div class="p-3">
                <h2 class="mb-2">{{ updateData ? 'Update' : 'Create' }} District</h2>
...
export default {
    ...
    methods:{
    	...
    	async storeOrUpdate () {
             // Submit the form.
            if(this.updateData) {
                const response = await this.form.put('/api/districts/'+this.form.id);
                if (response.status === 200) {
                    this.cleanForm();
                    showMessage(response.status, 'District updated successfully');
                }
            }else {
                const response = await this.form.post('/api/districts');
                if (response.status === 200) {
                    this.cleanForm();
                    showMessage(response.status, 'District created successfully');
                }
            }
        },
    ...
    created(){
        var self = this;
        axios.get('/api/districts/create').then(function (response) {
